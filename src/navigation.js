  ...
  routes[routeName] = createStream({
    ...
    onSinkComplete(payload, ctx) {
      ...
      if (!documents.length) {
        ...
        console.warn(
          `Navigation to route "${routeName}" ended without rendering any navigation record!`,
        );
      }
      ...
      if (documents.length > 1) {
        // eslint-disable-next-line max-len
        console.warn(
          `Navigation to route "${routeName}" ended with rendering more than one navigation record!`,
        );
      }
      ...
      if (documents[0].route !== routeName) {
        // eslint-disable-next-line max-len
        console.warn(
          `Navigation to route "${routeName}" ended with unexpected navigation document "${
            documents[0].route
          }"!`,
        );
      }
...
Object.keys(event).forEach(id => {
  ...
  if (typeof App.TVDML === 'undefined') App.TVDML = {};
  App.TVDML[name] = options => {
